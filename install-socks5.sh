...
source_config() {
    if [ -f $SOCKS_CONFIG ]; then
        # 使用grep和awk提取JSON值
        bind_address=$(grep -o '"bind_address":.*' $SOCKS_CONFIG | awk -F'"' '{print $4}')
        port=$(grep -o '"port":.*' $SOCKS_CONFIG | awk -F'"' '{print $4}')
        auth_needed=$(grep -o '"auth":.*' $SOCKS_CONFIG | awk -F'"' '{print $4}')
        username=$(grep -o '"username":.*' $SOCKS_CONFIG | awk -F'"' '{print $4}')
        password=$(grep -o '"password":.*' $SOCKS_CONFIG | awk -F'"' '{print $4}')
        udp_enabled=$(grep -o '"udp":.*' $SOCKS_CONFIG | awk -F'"' '{print $4}')
        
        # 设置默认值（如果配置文件中不存在）
        bind_address=${bind_address:-$DEFAULT_BIND}
        port=${port:-$DEFAULT_PORT}
        auth_needed=${auth_needed:-n}
...
SOCKS_BIN="/usr/local/bin/3proxy"
SOCKS_CONFIG_DIR="/etc/socks5"
